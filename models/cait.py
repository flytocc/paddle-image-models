...
ones_ = nn.initializer.Constant(1)
zeros_ = nn.initializer.Constant(0)
...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = (img_size, img_size) if isinstance(
            img_size, int) else img_size
        patch_size = (patch_size, patch_size) if isinstance(
...
class Attention_talking_head(nn.Layer):
    ...
    def forward(self, x):
        ...
        attn = self.proj_l(attn.transpose(
            [0, 2, 3, 1])).transpose([0, 3, 1, 2])
        attn = nn.functional.softmax(attn, axis=-1)

        attn = self.proj_w(attn.transpose(
            [0, 2, 3, 1])).transpose([0, 3, 1, 2])
        attn = self.attn_drop(attn)
...
class CaiT(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = self.create_parameter([1, num_patches, embed_dim])
...
def cait_XXS24_224(**kwargs):
    model = CaiT(
        img_size=224, patch_size=16, embed_dim=192, depth=24, num_heads=4,
        mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, epsilon=1e-6),
        drop_path_rate=0.05, init_scale=1e-5, depth_token_only=2, **kwargs)
    return model
