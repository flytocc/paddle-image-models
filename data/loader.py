...
from .sampler import DistributedRepeatedAugBatchSampler
...
def create_loader(
    ...
    shuffle = drop_last = is_training and not debug
    if distributed and not isinstance(dataset, IterableDataset):
        if is_training:
            if num_aug_repeats:
                batch_sampler = DistributedRepeatedAugBatchSampler(
                    dataset, batch_size, shuffle=shuffle, drop_last=drop_last, num_repeats=num_aug_repeats)
            else:
